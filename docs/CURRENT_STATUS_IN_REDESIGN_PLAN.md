...
- [ ] Test-typecheck gate (hardening, found during Finding 4): **partial 2026-07-07** — `npm run typecheck:tests` (`tsconfig.tests.json`) added; the gate exists and runs. Burned residual **40 → 23** via four genuine library fixes (`Behavior.make` generic defaults, `View.Attribute.AriaDescribedby`, `View.isElement/isFragment/isTextNode` guards, `Component.renderEffect` carrying the `SlotContract` axis) plus clean test narrowing. The gate **surfaced a real bug**: `Atom.family` overload ordering mis-resolves when the family fn returns an atom (details in archive log) — validates the gate's worth. Remaining 23 are dominated by legacy-pattern drift (deprecated `attachByView`, bindings-as-slots, old route API) → **fold the burndown into the Finding-3/P6 test-migration pass**, then make `typecheck:tests` a required gate.
